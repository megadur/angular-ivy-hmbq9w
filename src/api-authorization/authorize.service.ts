...
export class AuthorizeService {
  ...
  private userSubject: BehaviorSubject<IUser | Profile | null> = new BehaviorSubject<IUser | Profile | null>(null);

  public isAuthenticated(): Observable<boolean> {
    return this.getUser().pipe(map(u => !!u));
  }

  public getUser(): Observable<IUser | Profile| null> {
    return concat(
  ...
  public async signIn(state: any): Promise<IAuthenticationResult> {
    ...
    try {
      ...
      try {
        ...
        if (popupError === 'Popup window closed') {
  ...
  public async completeSignOut(url: string): Promise<IAuthenticationResult| undefined> {
    await this.ensureUserManagerInitialized();
    try {
      ...
      if (state) {
        ...
        return this.error('state authentication error.');
      }
    } catch (signInResponseError) {
  ...
  private getUserFromStorage(): Observable<IUser|Profile| null> {
    return from(this.ensureUserManagerInitialized())
      .pipe(
